...
class SwaggerModule {
    ...
    static shouldRun() {
        const env_check = env_1.ENV.ENVIRONMENT === 'prod' ? this.docs_on_prod : true;
        return env_check;
    }
    static setup(props) {
        if (props.enable_on_prod)
            this.docs_on_prod = props.enable_on_prod;
        if (!this.shouldRun())
            return;
    ...
    static logs() {
        if (!this.shouldRun() || !this.doc_route || !this.port)
            return;
        console.log(`[DOCS] | Swagger UI is available at http://localhost:${this.port}${this.doc_route}`);
        console.log(`[DOCS] | Documentation schema is available at http://localhost:${this.port}${this.doc_route}-json`);
    }
}
